...
TAC* tacGenerate(Ast *node)
{
	...
	switch(node->type)
	{
		...
		case AST_VECTOR: 	result = tacJoin(code[0], tacCreate(TAC_VECTOR, node->son[0]->symbol, code[1]?code[1]->res:0, 0)); break;

		case AST_ADD: 		return makeBinOp(code[0],code[1],TAC_ADD);break;
		case AST_SUB: 		return makeBinOp(code[0],code[1],TAC_SUB);break;
		case AST_MULT: 		return makeBinOp(code[0],code[1],TAC_MUL);break; 
		case AST_DIV: 		return makeBinOp(code[0],code[1],TAC_DIV);break; 
		case AST_LESS: 		return makeBinOp(code[0],code[1],TAC_L);break;
		case AST_GREATER: 	return makeBinOp(code[0],code[1],TAC_G);break;
		case AST_LE: 		return makeBinOp(code[0],code[1],TAC_LE);break;
		case AST_GE: 		return makeBinOp(code[0],code[1],TAC_GE);break; 
		case AST_EQUAL: 	return makeBinOp(code[0],code[1],TAC_EQ);break; 
		case AST_AND: 		return makeBinOp(code[0],code[1],TAC_AND);break; 
		case AST_OR: 		return makeBinOp(code[0],code[1],TAC_OR);break; 
		case AST_NEG: 		result = tacJoin(tacJoin(code[0], code[1]),tacCreate(TAC_NOT, makeTemp(), code[0]?code[0]->res:0, code[1]?code[1]->res:0)); break; 

		case AST_ATRIBUICAO_VEC: result = tacJoin(tacJoin(code[0],code[2]),tacCreate(TAC_MOVE_VEC,code[0]?code[0]->res:0,code[1]?code[1]->res:0,code[2]?code[2]->res:0)); break;
		...
		case AST_CALL_FUNCTION: return tacJoin(code[1], tacCreate(TAC_FUNCALL,makeTemp(),node->son[0]->symbol,0)); break;
		case AST_LISTEXP: return tacJoin(tacJoin(code[0],tacCreate(TAC_ARG,node->son[0]->symbol,0,0)),code[1]); break;

		case AST_FUNCAO: 	result = makeFunction(tacCreate(TAC_SYMBOL, node->son[0]->son[1]->symbol, 0, 0), code[1], code[2]); break;
